    ...
    for index in range(len(image_list)):
        ...
        if im_bboxes_num > 0:
            ...
            for idx in range(im_bboxes_num):
                # #在thief和evil图片中屏蔽锥桶的预测框
                # if len(image_id) >= 5 and image_id[:5] == "thief" and (int(id_results[idx]) + 1) == 4:
                #     continue
                # if len(image_id) >= 4 and image_id[:4] == "evil" and (int(id_results[idx]) + 1) == 4:
                #     continue
